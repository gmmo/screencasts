...
namespace soft_body
{
    ...
    {
        m_sphereCenter = XMFLOAT3(0.f, 0.f, 0.f);
    ...
    void ParticleSystem::SatisfyConstraints()
    {
        ...
        for (int j = 0; j < NUM_ITERATIONS; j++)
        {
            ...
            for (uint32_t ii = 0; ii < NUM_PARTICLES; ii++)
            {
                ...
                contactNormal.x = p.x - m_sphereCenter.x;
                contactNormal.y = p.y - m_sphereCenter.y;
                contactNormal.z = p.z - m_sphereCenter.z;
                ...
                if (deltalength < sphereRadius)
                {
                    ...
                    m_x[ii].x = m_sphereCenter.x + (sphereRadius * contactNormal.x);
                    m_x[ii].y = m_sphereCenter.y + (sphereRadius * contactNormal.y);
                    m_x[ii].z = m_sphereCenter.z + (sphereRadius * contactNormal.z);
                }
            }
        }
    }

    void ParticleSystem::UpdateSphereCenter(float x, float y, float z)
    {
        m_sphereCenter.x = x;
        m_sphereCenter.y = y;
        m_sphereCenter.z = z;
